  ...
  const id = req.body.timesheet.id,
      ...
      db.run(sql, values, function(error) {
        if (error) {
          ...
          db.get(`SELECT * FROM Iimesheet WHERE Timesheet.id = ${this.lastID}`,
            (error, timesheet) => {
...
timesheetsRouter.put('/:timesheetId', (req, res, next) => {
  const id = req.body.timesheet.id,
      ...
      const sql = 'UPDATE Timesheet SET id = $id, hours = $hours, ' +
          'rate = $rate, date = $date ' +
